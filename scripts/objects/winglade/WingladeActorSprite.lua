...
function WingladeActorSprite:init(actor)
    ...
    self.top_bit = Sprite(self:getTexturePath("top_bit"))
    self.top_bit:setScaleOriginExact(30, 26)
    self.top_bit.debug_select = false
    self:addChild(self.top_bit)

    self.sword = Sprite(self:getTexturePath("sword"))
    self.sword:setScaleOriginExact(30, 26)
    self.sword.debug_select = false
    self:addChild(self.sword)

    self.left_wing = Sprite(self:getTexturePath("left_wing"))
    self.left_wing:setScaleOriginExact(30, 26)
    self.left_wing:play(1/6)
    self.left_wing.debug_select = false
    self:addChild(self.left_wing)

    self.right_wing = Sprite(self:getTexturePath("right_wing"))
    self.right_wing:setScaleOriginExact(30, 26)
    self.right_wing:play(1/6)
    self.right_wing.debug_select = false
    self:addChild(self.right_wing)

    self.black = Sprite(self:getTexturePath("black"))
    self.black:setScaleOriginExact(30, 26)
    self.black.debug_select = false
    self:addChild(self.black)

    self.halo = Sprite(self:getTexturePath("halo"))
    self.halo:setScaleOriginExact(30, 26)
    self.halo.debug_select = false
    self:addChild(self.halo)

    self.eye_white = Sprite(self:getTexturePath("eye_white"))
    self.eye_white:setScaleOriginExact(30, 26)
    self.eye_white.debug_select = false
    self:addChild(self.eye_white)

    self.eye_pupil = Sprite(self:getTexturePath("eye_pupil"))
    self.eye_pupil:setScaleOriginExact(30, 26)
...
function WingladeActorSprite:setAnimation(anim, callback, ignore_actor_callback)
    local animations = { "idle", "spared", "retract", "retract_shoot", "stare" }
...
function WingladeActorSprite:update()
    ...
    if anim == "idle" or anim == "spared" then
        ...
        self.eye_white.x = MathUtils.lerp(self.eye_white.x, self.eye_target_x / 5, 0.2 * DTMULT)
        self.eye_white.y = MathUtils.lerp(self.eye_white.y, self.eye_target_y / 5, 0.2 * DTMULT)
        self.eye_pupil.x = MathUtils.lerp(self.eye_pupil.x, self.eye_target_x, 0.2 * DTMULT)
        self.eye_pupil.y = MathUtils.lerp(self.eye_pupil.y, self.eye_target_y, 0.2 * DTMULT)
    end
    if anim == "spared" then
        self.eye_pupil.visible = false
        self.eye_white:setSprite(self:getTexturePath("eye_pupil_spare"))
    ...
    if anim == "retract" or anim == "retract_shoot" or anim == "stare" then
        ...
        x_screen_pos, y_screen_pos = x_screen_pos, y_screen_pos
